...
\documentclass[a0paper,portrait,fontscale=0.42]{tikzposter}
...
\posterfig{0.95\linewidth}{fig_range_doppler_fft.png}{Range-Doppler recovery via FFT (fast-time/slow-time), with resolution formulas}
\posterfig{0.95\linewidth}{fig_cfar_pipeline.png}{CFAR detection: cell-under-test, guard cells, training cells}
\posterfig{\linewidth}{fig_mimo_antenna_array.png}{Physical $\to$ virtual MIMO antenna array (azimuth + elevation)}
\posterfig{\linewidth}{03_preprocessing.png}{Point-cloud preprocessing: DBSCAN $\to$ near-field filter $\to$ normalize $\to$ pad}
\posterfig{0.9\linewidth}{fig_dbscan_before_after.png}{DBSCAN removes multipath ghosts: 6 clusters $\to$ 1}
}
...
\posterfig{\linewidth}{05_pcttrans_decomposed.png}{Layer decomposition: Point/Neighbor Embedding $\to$ Offset-Attention $\to$ MA-Pool $\to$ Transformer}
\posterfig{0.95\linewidth}{fig_pretrained_maml.png}{Pre-trained ($\theta^{PT}_0$) vs.\ random ($\theta^{rand}_0$) initialization within task distribution; inner/outer-loop adaptation}
\posterfig{\linewidth}{06_pretrained_maml.png}{Two-phase algorithm: supervised pre-training $\to$ MAML meta-training}
}
