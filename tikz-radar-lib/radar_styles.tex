...
\colorlet{deepLearning}{radarPurple}     % NN / learning blocks
\colorlet{resultsColor}{radarGreen}    % results / metrics
\colorlet{geometryColor}{radarOrange}   % geometry / point-cloud blocks
...
% ------------------------------------------------------------
% 1b. LIGHT PALETTE  (pastel tints, for content diagrams --
%     process cards, icon panels, legends -- to sit visually
%     alongside lighter figures like the PCTtrans architecture
%     diagram, rather than reading as a heavier, separate style.
%     The bold saturated colors above stay in use for structural
%     UI like poster block headers via \SectionHeader.)
% ------------------------------------------------------------
% Fill tint for a card/shape body (very light, ~14% color on white)
\newcommand{\lightFill}[1]{#1!16!white}
% Fill tint for a title bar / accent band (~32% color on white --
% enough to read as "that module's color" without going saturated)
\newcommand{\midFill}[1]{#1!32!white}
% Text on a light/mid fill always stays dark navy, never white,
% since white-on-pastel has poor contrast.
\colorlet{cardTitleText}{radarNavy}

\colorlet{radarBlueLight}{radarBlue!32!white}
\colorlet{radarGreenLight}{radarGreen!32!white}
\colorlet{radarOrangeLight}{radarOrange!32!white}
\colorlet{radarPurpleLight}{radarPurple!32!white}
\colorlet{radarGreyLight}{radarGrey!20!white}
...
\tikzset{
  process card/.style = {
    ...
    rectangle split part fill={white, radarProcessing!32!white},
  },
  % Safe way to recolor a single card, e.g.:
  %   \ProcessCard[card color=deepLearning]{b}{Range FFT}{...}
  % Produces a light pastel tint of the given color for the title
  % band (~32%) rather than a fully saturated fill, and keeps the
  % icon panel white (icons already paint their own white/bordered
  % background, so tinting nodepart-one would be invisible anyway).
  % (Do not set `fill=` directly on a process card -- it conflicts
  % with `rectangle split part fill` and blanks the node.)
  card color/.style = {rectangle split part fill={white, #1!32!white}},
  % Opt back into the old fully-saturated banner + white text look
  % for a single card, e.g. \ProcessCard[card color bold=deepLearning]{...}
  card color bold/.style = {rectangle split part fill={white, #1}},
}
...
\newcommand{\ProcessCard}[4][]{%
  \node[process card, #1] (#2) {%
    \nodepart{one}
      ...
      \end{tikzpicture}
    \nodepart{two}
      \textcolor{cardTitleText}{\sffamily\bfseries\footnotesize #3}
  };%
}
...
% Swatch is a light tint of the given color with a thin dark
% border, matching the process-card title bands.
\newcommand{\LegendSwatch}[3]{%
  \begin{scope}[shift={(#1)}]
    \node[draw=iconStroke, line width=0.5pt, fill=#2!32!white, rounded corners=1.5pt,
          minimum width=10pt, minimum height=10pt, anchor=west] (sw) {};
    \node[anchor=west, font=\sffamily\footnotesize, text=radarGrey, right=4pt of sw] {#3};
  \end{scope}
}
% Usage: \LegendBox{x}{y}{color}{label}  (explicit x,y instead of a coordinate tuple)
\newcommand{\LegendBox}[4]{%
  \begin{scope}[shift={(#1,#2)}]
    \node[draw=iconStroke, line width=0.5pt, fill=#3!32!white, rounded corners=1.5pt,
          minimum width=8pt, minimum height=8pt, anchor=west] (sw) at (0,0) {};
...
% Header row: dark navy bold text on a light radarProcessing tint
% band. Formatting resets after \midrule so it doesn't bleed into
% the data rows below.
\newcommand{\HeaderRow}[1]{%
  \rowcolor{radarProcessing!32!white}\bfseries\color{radarNavy}#1\\
  \midrule
